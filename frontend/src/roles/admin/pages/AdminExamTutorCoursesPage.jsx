...
export default function AdminExamTutorCoursesPage() {
  ...
  return (
    <div className="aes-page">
      <button type="button" className="aes-back" onClick={() => navigate('/admin/exams')}>
        ← กลับไปหน้ารายชื่อติวเตอร์
      </button>

      <div className="aes-header">
        <div>
          <h1>คะแนนสอบ · {tutorName || 'ติวเตอร์'}</h1>
          <p>
            {tutor?.tutorCode ? `${tutor.tutorCode} · ` : ''}
            เลือกคอร์สเพื่อดูคะแนนสอบของนักเรียนในคอร์สนั้น (ดูได้อย่างเดียว)
          </p>
        </div>
      </div>
        ...
        <div className="aes-grid">
          {filtered.map((course) => (
            <button
              ...
              type="button"
              className="aes-card"
              onClick={() => navigate(`/admin/exams/tutors/${tutorId}/courses/${course.id}`)}
            >
