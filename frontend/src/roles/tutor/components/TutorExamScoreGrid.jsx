...
import '../pages/TutorExamScoreCoursePage.css';
...
// ตารางคะแนนสอบ (แก้ไขได้) ของคอร์สหนึ่ง — ใช้ร่วมกันทั้งหน้าเต็ม (TutorExamScoreCoursePage)
// และโมดัลในหน้าประวัติคอร์สเรียน (TutorCourseHistoryPage)
export default function TutorExamScoreGrid({ courseId }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setEdits({});
      setCellState({});
      const [examList, manualList, enrollmentList] = await Promise.all([
  ...
  if (loading) {
    return <div className="escd-empty">กำลังโหลดคะแนน...</div>;
  }
