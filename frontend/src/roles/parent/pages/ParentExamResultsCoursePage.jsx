...
export default function ParentExamResultsCoursePage() {
  ...
  const studentName = scores[0]?.studentName || course?.studentName || 'บุตรหลาน';
  ...
  return (
    <div className="aes-page">
      <button type="button" className="aes-back" onClick={() => navigate('/parent/exam-results')}>
        ← กลับไปหน้าผลการสอบ
      </button>
      ...
      {loading ? (
        ...
        <div className="aes-table-card">
          ...
          <div className="aes-legend">
            <span>คุณดูคะแนนสอบของบุตรหลานได้เท่านั้น — การกรอกคะแนนทำได้ที่บัญชีติวเตอร์</span>
          </div>
        </div>
      )}
    </div>
