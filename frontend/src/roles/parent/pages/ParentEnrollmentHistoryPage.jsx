...
import '../../student/pages/StudentEnrollmentHistoryPage.css';
...
// ป้ายสถานะฝั่งผู้ปกครองใช้คำว่า "รอตรวจสอบสลิป" แทน "รอการยืนยันชำระเงิน" เหมือนฝั่งนักเรียน
const PARENT_STATUS_LABEL = { ...ENROLLMENT_HISTORY_STATUS_LABEL, PENDING_VERIFICATION: 'รอตรวจสอบสลิป' };

function statusBadge(key) {
  return { label: PARENT_STATUS_LABEL[key], cls: STATUS_CLS[key] };
}

const FILTERS = [
  { key: 'ALL', label: 'ทั้งหมด' },
  ...ENROLLMENT_HISTORY_STATUSES.map((key) => ({ key, label: PARENT_STATUS_LABEL[key] })),
];

const getDisplayStatus = getEnrollmentHistoryStatus;

export default function ParentEnrollmentHistoryPage() {
  ...
  async function load(showLoading = false) {
    ...
    try {
      const data = await getChildEnrollments().catch(() => []);
    ...
    <div className="hist-page">
      <div className="hist-header">
        <div>
          ...
          <p>ตรวจสอบสถานะการสมัครเรียนและการชำระเงินของบุตรหลาน</p>
        </div>
      </div>
