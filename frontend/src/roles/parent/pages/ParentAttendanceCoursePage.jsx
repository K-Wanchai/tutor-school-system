...
export default function ParentAttendanceCoursePage() {
  ...
  const load = useCallback(async () => {
    ...
    const [courseR, sessionR, recordR] = await Promise.allSettled([
      getChildEnrollments(),
      getChildCourseSessions(courseId),
      getChildAttendance(),
    ]);
    const errs = [];
    const take = (r, label) => {
  ...
  const studentName = records[0]?.studentName || course?.studentName || 'บุตรหลาน';
  ...
  return (
    <div className="aes-page">
      <button type="button" className="aes-back" onClick={() => navigate('/parent/attendance')}>
      ...
      {loading ? (
        ...
        <div className="aes-table-card">
          ...
          <div className="aes-legend">
            ...
            <span>คุณดูข้อมูลการเข้าเรียนของบุตรหลานได้เท่านั้น — การเช็คชื่อทำได้ที่บัญชีติวเตอร์</span>
          </div>
        </div>
      )}
    </div>
